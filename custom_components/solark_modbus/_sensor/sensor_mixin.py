...
class SensorMixin(Generic[TSensorValue]):
    ...
    DynamicIconAndValue: dict[TSensorValue, Tuple[Any, str]] | None = None

    @classmethod
    def _get_dynamic_entry(
        cls, lookup_map_key: TSensorValue
    ) -> tuple[Any, str] | None:
        ''' Return the dictionary entry for the given key if DynamicIconAndValue is configured. '''
        d = cls.DynamicIconAndValue
        if not d:
            return None

        if lookup_map_key is None:
            raise ValueError(
                f"Value {lookup_map_key!r} is None. DynamicIconAndValue keys: {list(d.keys())}"
            ) from None
        ...
        try:
            ...
            raise ValueError(
                f"Value {lookup_map_key!r} not valid for DynamicIconAndValue keys: {list(d.keys())}"
            ) from None
